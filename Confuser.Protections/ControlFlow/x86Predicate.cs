	...
	internal class x86Predicate : IPredicate {
		...
		class x86Encoding {
			...
			public void Compile(CFContext ctx) {
				...
				native = new MethodDefUser(ctx.Context.Registry.GetService<NameService>().RandomName(), MethodSig.CreateStatic(int32, int32), MethodAttributes.PinvokeImpl | MethodAttributes.PrivateScope | MethodAttributes.Static);
