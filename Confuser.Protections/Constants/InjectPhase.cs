...
namespace Confuser.Protections.Constants {
	internal class InjectPhase : ProtectionPhase {
		...
		void InjectHelpers(ConfuserContext context, ICompressionService compression, IRuntimeService rt, CEContext moduleCtx) {
			IEnumerable<IDnlibDef> members = Core.Helpers.InjectHelper.Inject(rt.GetRuntimeType("Confuser.Runtime.Constant"), context.CurrentModule.GlobalType, context.CurrentModule);
			...
			for (int i = 0; i < moduleCtx.DecoderCount; i++) {
				MethodDef decoderInst = Core.Helpers.InjectHelper.Inject(decoder, context.CurrentModule);
