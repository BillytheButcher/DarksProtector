	...
	internal class StrongMode : RPMode {
		...
		TypeDef GetKeyAttr(RPContext ctx) {
			...
			if (keyAttrs[index] == null) {
				...
				TypeDef injectedAttr = Core.Helpers.InjectHelper.Inject(rtType, ctx.Module);
				injectedAttr.Name = ctx.Name.RandomName();
				injectedAttr.Namespace = ctx.Name.RandomName();
		...
		InitMethodDesc GetInitMethod(RPContext ctx, IRPEncoding encoding) {
			...
			if (initDescs[index] == null) {
				...
				MethodDef injectedMethod = Core.Helpers.InjectHelper.Inject(rtType.FindMethod("Initialize"), ctx.Module);
